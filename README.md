# Hi there 👋 I'm Rhishikesh

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=26&duration=3000&pause=1000&color=36BCF7&width=900&lines=Senior+Python+Backend+Engineer;FastAPI+%7C+AWS+%7C+Kubernetes;Building+Production-Grade+Backend+Systems;Cloud-Native+Microservices+Developer;Distributed+Systems+%26+DevOps)](https://git.io/typing-svg)

<p align="left">
  <img
    src="https://komarev.com/ghpvc/?username=rhishikesh-chikhalkar&label=Profile%20Views&color=0e75b6&style=flat"
    alt="Profile Views"
  />
</p>

---

## 🚀 About Me

Senior Python Backend Engineer with 4+ years of experience building scalable cloud-native backend systems using FastAPI, Django, PostgreSQL, Docker, Kubernetes, and AWS.

Experienced in designing production-grade microservices, optimizing high-volume financial data workflows, deploying applications on AWS EKS, and debugging distributed systems in Kubernetes environments.

### Key Achievements

- Improved API response times by ~40% through database and query optimization
- Reduced production incident resolution time by ~30% using observability and Kubernetes debugging tools
- Built scalable FastAPI microservices for financial portfolio analytics systems
- Recognized as Infosys Platinum Club Member (Top 3% Performer)
- Designed CI/CD deployment workflows using AWS CodePipeline and GitHub Actions

---

## 🛠 Tech Stack

#### Backend
![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white)
![Python](https://img.shields.io/badge/Python-3670A0?style=flat&logo=python&logoColor=ffdd54)
![Django](https://img.shields.io/badge/Django-092E20?style=flat&logo=django&logoColor=white)
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![SQLAlchemy](https://img.shields.io/badge/SQLAlchemy-D71F00?style=flat&logo=sqlalchemy&logoColor=white)

#### Cloud & DevOps
![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat&logo=amazon-aws&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?style=flat&logo=kubernetes&logoColor=white)
![Terraform](https://img.shields.io/badge/Terraform-5835CC?style=flat&logo=terraform&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat&logo=github-actions&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=flat&logo=nginx&logoColor=white)

#### Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=flat&logo=mongodb&logoColor=white)
![Snowflake](https://img.shields.io/badge/Snowflake-29B5E8?style=flat&logo=snowflake&logoColor=white)

#### Data & Monitoring
![Pandas](https://img.shields.io/badge/Pandas-150458?style=flat&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white)
![Splunk](https://img.shields.io/badge/Splunk-000000?style=flat&logo=splunk&logoColor=white)
![Apache Airflow](https://img.shields.io/badge/Airflow-017CEE?style=flat&logo=apacheairflow&logoColor=white)
![CloudWatch](https://img.shields.io/badge/CloudWatch-FF4F8B?style=flat&logo=amazoncloudwatch&logoColor=white)

#### Tools & Platforms
![Git](https://img.shields.io/badge/Git-F05033?style=flat&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-121011?style=flat&logo=github&logoColor=white)
![Bitbucket](https://img.shields.io/badge/Bitbucket-0047B3?style=flat&logo=bitbucket&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)
![Databricks](https://img.shields.io/badge/Databricks-FF3621?style=flat&logo=databricks&logoColor=white)

---

## 📌 Featured Production Work

### 🚀 Portfolio Metrics API

Production-grade financial analytics platform built using FastAPI and PostgreSQL for portfolio performance tracking, benchmark comparisons, and real-time metrics computation.

#### Highlights

- Designed async FastAPI microservices for high-volume financial workloads
- Optimized PostgreSQL queries improving API response times by ~40%
- Integrated third-party market and benchmark APIs
- Implemented OAuth2 authentication and secure API workflows
- Built CI/CD deployment pipelines using AWS CodePipeline & GitHub Actions
- Deployed containerized services on AWS EKS using Docker & Kubernetes
- Production monitoring using Lens, Aptakube, and CloudWatch

#### Tech Used

`Python` `FastAPI` `PostgreSQL` `AWS EKS` `Docker` `Kubernetes`

---

### ☁️ Cloud-Native Backend Platform

Scalable backend platform designed for Kubernetes deployments, observability, infrastructure automation, and production-grade cloud operations.

#### Highlights

- Kubernetes-ready deployment architecture
- Production logging & monitoring
- Infrastructure automation workflows
- Environment-based configurations
- Distributed service deployments
- API-first backend architecture
- Scalable cloud-native workflows

#### Tech Used

`Python` `FastAPI` `AWS` `Docker` `Kubernetes`

---

### 🤖 AI Backend Integrations

Backend systems integrating LLM APIs, workflow automation, and scalable AI orchestration services using Python and cloud-native deployment patterns.

#### Focus Areas

- AI API orchestration
- Workflow automation
- Async processing systems
- Scalable AI services
- Cloud deployment patterns
- AI-integrated backend workflows

---

## ⚙️ Production Engineering

Experienced in:

- Kubernetes production debugging
- Cloud-native deployments on AWS EKS
- CI/CD automation workflows
- Performance optimization & scalability tuning
- API observability & monitoring
- Secure backend architecture
- Distributed systems troubleshooting
- Production incident resolution

---

## 🏆 Recognition

🥇 Infosys Platinum Club Member (FY 2025–26)

Recognized among the top 3% performers across Infosys for technical excellence, ownership, delivery quality, and engineering impact.

---

## 📚 Currently Working With

- FastAPI microservices
- Kubernetes & AWS EKS
- Distributed backend systems
- Async Python processing
- AI-integrated backend services
- Production observability workflows
- System design & scalability patterns

---

## 📈 GitHub Stats

<p align="left">
  <img
    src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=rhishikesh-chikhalkar&theme=tokyonight"
    width="450"
  />
</p>

---

## 🔥 Contribution Streak

<p align="left">
  <img
    src="https://streak-stats.demolab.com?user=rhishikesh-chikhalkar&theme=tokyonight"
  />
</p>

---

## 📊 GitHub Activity Graph

[![Rhishikesh's github activity graph](https://github-readme-activity-graph.vercel.app/graph?username=rhishikesh-chikhalkar&theme=tokyo-night)](https://github.com/ashutosh00710/github-readme-activity-graph)

---

## 🏆 GitHub Trophies

[![trophy](https://github-profile-trophy.vercel.app/?username=rhishikesh-chikhalkar&theme=tokyonight&margin-w=15)](https://github.com/ryo-ma/github-profile-trophy)

---

## 🐍 Contribution Snake

![Snake animation](https://raw.githubusercontent.com/rhishikesh-chikhalkar/rhishikesh-chikhalkar/output/github-contribution-grid-snake.svg)

---

## 🤝 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/rhishikesh-chikhalkar)

[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=flat&logo=gmail&logoColor=white)](mailto:rhishichikhalkar21@gmail.com)

---

⭐ If you like my work, consider starring my repositories.
